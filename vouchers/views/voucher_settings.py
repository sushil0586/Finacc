...
POLICY_CONTROLS_SCHEMA = [
    ...
    {"key": "voucher_maker_checker", "label": "Voucher Maker Checker", "type": "choice", "choices": ["off", "warn", "hard"]},
    ...
    {"key": "allow_control_account_lines", "label": "Allow Control Account Lines", "type": "choice", "choices": ["on", "off"]},
    {"key": "require_cash_bank_account_for_cash_bank", "label": "Require Cash/Bank Account For Cash/Bank", "type": "choice", "choices": ["on", "off"]},
    {"key": "cash_bank_mixed_entry_rule", "label": "Cash/Bank Mixed Entry Rule", "type": "choice", "choices": ["off", "hard"]},
]

VOUCHER_SETTINGS_SCHEMA = [
    {"name": "default_doc_code_cash", "label": "Cash Voucher Code", "type": "string", "group": "numbering_defaults"},
    {"name": "default_doc_code_bank", "label": "Bank Voucher Code", "type": "string", "group": "numbering_defaults"},
    {"name": "default_doc_code_journal", "label": "Journal Voucher Code", "type": "string", "group": "numbering_defaults"},
...
VOUCHER_DOC_TYPES = [
    {
        ...
        "default_code_field": "default_doc_code_cash",
        "fallback_code": "CV",
    },
    {
        "series_key": "bank_voucher",
        "doc_key": "BANK_VOUCHER",
        "name": "Bank Voucher",
        "voucher_type": VoucherHeader.VoucherType.BANK,
        "default_code_field": "default_doc_code_bank",
        "fallback_code": "BV",
    },
    {
        "series_key": "journal_voucher",
        "doc_key": "JOURNAL_VOUCHER",
        "name": "Journal Voucher",
        "voucher_type": VoucherHeader.VoucherType.JOURNAL,
        "default_code_field": "default_doc_code_journal",
        "fallback_code": "JV",
    },
]


class VoucherSettingsAPIView(ScopedEntitlementMixin, APIView):
    ...
    def _list_lock_periods(self, *, entity_id: int, subentity_id: Optional[int]) -> list[dict]:
        qs = VoucherLockPeriod.objects.filter(entity_id=entity_id)
    ...
    def _replace_lock_periods(self, rows: list[dict], *, entity_id: int, subentity_id: Optional[int]) -> None:
        ...
        for row in rows:
            ...
            VoucherLockPeriod.objects.create(entity_id=entity_id, subentity_id=subentity_id, lock_date=row["lock_date"], reason=row.get("reason") or "")

    def _list_choice_overrides(self, *, entity_id: int, subentity_id: Optional[int]) -> list[dict]:
        catalog = VoucherChoiceService.compile_choices(entity_id=entity_id, subentity_id=subentity_id)
        valid_keys = self._valid_override_keys(catalog)
        qs = VoucherChoiceOverride.objects.filter(entity_id=entity_id)
        qs = qs.filter(subentity__isnull=True) if subentity_id is None else qs.filter(subentity_id=subentity_id)
        rows = []
    ...
    def _replace_choice_overrides(self, rows: list[dict], *, entity_id: int, subentity_id: Optional[int]) -> None:
        ...
        for row in rows:
            ...
            VoucherChoiceOverride.objects.create(
                entity_id=entity_id,
    ...
    def _get_doc_type(doc_key: str, name: str, default_code: str):
        return ensure_document_type(module="vouchers", doc_key=doc_key, name=name, default_code=default_code)

    def _series_payload(self, *, entity_id: int, entityfinid_id: int, subentity_id: Optional[int], settings_obj) -> list[dict]:
        rows = []
        for cfg in VOUCHER_DOC_TYPES:
    ...
    def _update_numbering_series(self, rows: list[dict], *, entity_id: int, entityfinid_id: int, subentity_id: Optional[int], settings_obj, user_id: Optional[int]) -> None:
        by_key = {row.get("series_key"): row for row in rows if isinstance(row, dict)}
        for cfg in VOUCHER_DOC_TYPES:
            row = by_key.get(cfg["series_key"])
            if not row:
                continue
            doc_code = (row.get("doc_code") or getattr(settings_obj, cfg["default_code_field"]) or cfg["fallback_code"]).strip()
            if not doc_code:
                raise ValidationError({"numbering_series": f"doc_code is required for {cfg['series_key']}."})
    ...
    def _current_doc_numbers(self, *, entity_id: int, entityfinid_id: Optional[int], subentity_id: Optional[int]) -> Optional[dict]:
        if not entityfinid_id:
            return None
        current = {}
        for cfg in VOUCHER_DOC_TYPES:
            current[cfg["series_key"]] = VoucherSettingsService.current_doc_no_for_type(
                entity_id=entity_id,
                entityfinid_id=entityfinid_id,
                subentity_id=subentity_id,
                voucher_type=cfg["voucher_type"],
    ...
    def _payload(self, *, entity_id: int, subentity_id: Optional[int], entityfinid_id: Optional[int]) -> dict:
        ...
        choice_catalog = VoucherChoiceService.compile_choices(entity_id=entity_id, subentity_id=subentity_id)
        return {
            "settings": {
                ...
                "default_workflow_action": VoucherSettingsService.normalize_workflow_action(settings_obj.default_workflow_action),
                "policy_controls": VoucherSettingsService.effective_policy_controls(settings_obj),
            },
            "schema": VOUCHER_SETTINGS_SCHEMA,
            "policy_controls_schema": POLICY_CONTROLS_SCHEMA,
            "sections": _sections(VOUCHER_SETTINGS_SCHEMA),
            "current_doc_numbers": self._current_doc_numbers(entity_id=entity_id, entityfinid_id=entityfinid_id, subentity_id=subentity_id),
    ...
    def patch(self, request):
        ...
        try:
            updated = VoucherSettingsService.upsert_settings(
                entity_id=entity_id,
                subentity_id=subentity_id,
...
class VoucherCompiledChoicesAPIView(APIView):
    ...
    def get(self, request):
        ...
        sub_raw = request.query_params.get("subentity_id", request.query_params.get("subentity"))

        entity_id = None
        subentity_id = None
        if entity_raw not in (None, "", "null"):
            try:
                entity_id = int(entity_raw)
            except (TypeError, ValueError):
                raise ValidationError({"entity_id": "entity_id must be an integer."})
        if sub_raw not in (None, "", "null"):
            try:
                subentity_id = int(sub_raw)
            except (TypeError, ValueError):
