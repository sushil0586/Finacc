...
from receipts.models import ReceiptVoucherAllocation, ReceiptVoucherHeader
...
class ReceiptVoucherPDFAPIView(APIView):
    ...
    def get(self, request, pk: int):
        ...
        qs = (
            ...
            .select_related("received_in", "received_from", "receipt_mode", "entity", "entityfinid", "subentity")
            .prefetch_related(
                "adjustments",
        ...
        elems.append(Paragraph(f"Receipt Voucher: {title}", styles["Title"]))
        elems.append(Spacer(1, 4 * mm))

        header_rows = [
            ["Voucher Date", str(voucher.voucher_date), "Status", voucher.get_status_display()],
            ["Receipt Type", voucher.get_receipt_type_display(), "Supply Type", voucher.get_supply_type_display()],
            ["Received In", getattr(voucher.received_in, "accountname", ""), "Received From", getattr(voucher.received_from, "accountname", "")],
            ["Receipt Mode", getattr(voucher.receipt_mode, "paymentmode", "") or "", "Reference No", voucher.reference_number or ""],
            ["Cash Paid", str(voucher.cash_received_amount), "Settlement Support", str(voucher.settlement_effective_amount)],
        ]
        ...
        for row in voucher.allocations.all():
            ...
            alloc_rows.append([
                getattr(item, "invoice_number", "") or f"Open Item {getattr(item, 'id', '')}",
                getattr(item, "customer_reference_number", "") or "",
        ...
        for row in voucher.advance_adjustments.all():
            source_voucher = getattr(getattr(row, "advance_balance", None), "receipt_voucher", None)
            item = getattr(row, "open_item", None)
            adv_rows.append([
        ...
        total_rows = [
            ["Cash Paid", str(voucher.cash_received_amount)],
        ...
        doc.build(elems)
        filename = f"{voucher.voucher_code or f'receipt_voucher_{voucher.id}'}.pdf"
