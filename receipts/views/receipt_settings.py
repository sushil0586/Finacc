...
RECEIPT_DOC_TYPE = {
    ...
    "default_code_field": "default_doc_code_receipt",
    "fallback_code": "RV",
}
...
class ReceiptSettingsAPIView(APIView):
    ...
    def _validate_settings_updates(settings_updates: dict) -> None:
        workflow_values = {v for v, _ in ReceiptSettings.DefaultWorkflowAction.choices}
        if "default_doc_code_receipt" in settings_updates and settings_updates["default_doc_code_receipt"] is not None:
            code = str(settings_updates["default_doc_code_receipt"]).strip()
            if not code:
                raise ValidationError({"default_doc_code_receipt": "This field cannot be blank."})
            if len(code) > 10:
                raise ValidationError({"default_doc_code_receipt": "Ensure this value has at most 10 characters."})
    ...
    def _list_lock_periods(self, *, entity_id: int, subentity_id: Optional[int]) -> list[dict]:
        qs = ReceiptLockPeriod.objects.filter(entity_id=entity_id)
    ...
    def _list_choice_overrides(self, *, entity_id: int, subentity_id: Optional[int]) -> list[dict]:
        ...
        qs = ReceiptChoiceOverride.objects.filter(entity_id=entity_id)
        qs = qs.filter(subentity__isnull=True) if subentity_id is None else qs.filter(subentity_id=subentity_id)
        rows = []
    ...
    def _get_doc_type(doc_key: str, name: str, default_code: str):
        return ensure_document_type(module="receipts", doc_key=doc_key, name=name, default_code=default_code)

    def _series_payload(self, *, entity_id: int, entityfinid_id: int, subentity_id: Optional[int], settings_obj) -> list[dict]:
        cfg = RECEIPT_DOC_TYPE
    ...
    def _update_numbering_series(self, rows: list[dict], *, entity_id: int, entityfinid_id: int, subentity_id: Optional[int], settings_obj, user_id: Optional[int]) -> None:
        ...
        if not doc_code:
            raise ValidationError({"numbering_series": "doc_code is required for receipt_voucher."})
    ...
    def _current_doc_numbers(self, *, entity_id: int, entityfinid_id: Optional[int], subentity_id: Optional[int], settings_obj) -> Optional[dict]:
        if not entityfinid_id:
            return None
        return {
            "receipt_voucher": ReceiptSettingsService.get_current_doc_no(
                entity_id=entity_id,
                entityfinid_id=entityfinid_id,
                subentity_id=subentity_id,
                doc_key=RECEIPT_DOC_TYPE["doc_key"],
    ...
    def _payload(self, *, entity_id: int, subentity_id: Optional[int], entityfinid_id: Optional[int]) -> dict:
        ...
        choice_catalog = ReceiptChoiceService.compile_choices(entity_id=entity_id, subentity_id=subentity_id)
        return {
            "settings": {
