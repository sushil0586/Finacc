...
class ReceiptAllocationService:
    """Computes reference-aware, policy-driven allocatable AR rows for receipts."""
    ...
    @staticmethod
    def _open_items_queryset(*, entity_id: int, entityfinid_id: int, subentity_id: Optional[int], customer_id: int, is_open: bool = True):
        qs = CustomerBillOpenItem.objects.select_related("header", "header__original_invoice").filter(
            entity_id=entity_id,
    ...
    def _positive_doc_type(doc_type: Optional[int]) -> bool:
        # Sales: 1=invoice, 3=debit note are positive receivable types.
        ...
        *,
        ...
        for idx, item in enumerate(
            ...
            row = {
                ...
                "reference_invoice_header_id": getattr(getattr(item, "header", None), "original_invoice_id", None),
            }
            rows.append(row)
            if raw <= ZERO2:
