    ...
    def normalize_policy_controls(raw: Any) -> Dict[str, Any]:
        ...
        for key, value in raw.items():
            ...
            if key in {
                ...
                "auto_paisa_adjustment_on_receipt",
            }:
                if v not in ON_OFF:
                    raise ValueError(f"policy_controls.{key} must be one of: on, off.")
                normalized[key] = v
                continue
            if key == "paisa_adjustment_tolerance":
                try:
                    tolerance = float(value)
                except (TypeError, ValueError):
                    raise ValueError("policy_controls.paisa_adjustment_tolerance must be a number.")
                if tolerance < 0:
                    raise ValueError("policy_controls.paisa_adjustment_tolerance must be >= 0.")
                normalized[key] = f"{tolerance:.2f}"
                continue
        ...
        try:
            ...
            if subentity_id is not None and "Series not found" in str(e):
                ...
                if subentity_id is None and "Series not found" in reason:
                    reason = (
                        f"{reason}. Seed the root receipt voucher numbering scope for entity={entity_id}, fin={entityfinid_id}."
                    )
                return {
                    "enabled": False,
                    "reason": reason,
