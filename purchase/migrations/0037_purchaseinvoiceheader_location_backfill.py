...
def forwards(apps, schema_editor):
    ...
    for header in PurchaseInvoiceHeader.objects.filter(location__isnull=True).only("id", "entity_id", "subentity_id"):
        location_id = _pick_location(Godown, header.entity_id, header.subentity_id)
        if location_id:
            PurchaseInvoiceHeader.objects.filter(pk=header.pk).update(location_id=location_id)


def backwards(apps, schema_editor):
    # Intentionally preserve historical backfill data.
    return


class Migration(migrations.Migration):
    dependencies = [
        ("purchase", "0036_purchaseinvoiceheader_location"),
