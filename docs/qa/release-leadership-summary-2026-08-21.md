...
- Reports/regression chunk initially produced `15 passed`, `2 skipped`, and `6 failed`; after launch seed apply and external harness hardening, the stage-valid financial/compliance subset reran with `13 passed`, `5 skipped`, and `0 failed` in `4.6m`. Bank Reco stage-valid browser subset later reran clean with `21 passed`, `0 failed` in `3.9m`; the remaining Bank Reco mutation/integrity specs still shell into local Django and need matched-environment or remote-safe seed coverage.
- Onboarding P1 produced `2 passed`, `6 failed`; all failures timed out selecting District after state choice.
- Payroll P1 is not clean remote-stage proof yet because the current `Mehak-T` stage subscription blocks `feature_payroll`, and deeper payroll workflows still seed through local Django (`PAYROLL-001` to `PAYROLL-004` now report `1 passed`, `4 skipped`, `0 failed` after entitlement-aware skip hardening). Payment open-advance depth is now focused green after the seeded against-bill rerun (`FIN-VCH-030` with `2 passed` in `57.9s`).
...
- Stage apply succeeded for `Mehak-T`, `manav-t`, and `Ritikasharma`; post-seed browser proof on `Mehak-T` includes auth/entity integration `6 passed`, onboarding `7 passed` / `1 skipped`, compact Sales seed smoke `9 passed` / `1 skipped`, Sales TCS full flow `9 passed`, Purchase P0 coverage with `71 passed` initially plus focused `FIN-PUR-082` cleanup and post-P0 purchase note cluster `FIN-PUR-010B/020/021/023/024/026` rerun `7 passed` in `2.7m`, voucher/payables compact proof with `41 passed` in `16.5m`, focused payment open-advance `FIN-VCH-030` rerun `2 passed` in `57.9s`, public signup `FIN-REG-008` rerun `2 passed` in `29.5s`, clean full P0 stage evidence with `248 passed`, `30 skipped`, `0 failed` in `1.2h`, the stage-valid financial/compliance report subset with `13 passed`, `5 skipped`, `0 failed` in `4.6m`, and Bank Reco stage-valid browser subset with `21 passed`, `0 failed` in `3.9m`. Launch seed now also creates `LAUNCH_FREIGHT_18`; local backend charge round-trip guard, auth OTP delivery fallback, exact public-register mocked-SMTP guard, launch seed, and account-profile bundle passed with `11` tests.
