...
- A fresh timing run kept all 20 authenticated manufacturing report requests below `0.4 s`: Material Consumption p95/max `374 ms`, Output Yield `298 ms`, Posting Audit `295 ms`, and WIP/Cost Summary `288 ms`.
- The original three-pass browser rerun completed `46/52` in 10.9 minutes. All six failures occurred during page navigation/bootstrap while the report APIs remained fast. Staging was serving multi-megabyte Angular bundles without JavaScript/CSS compression or immutable caching for content-hashed chunks.
- Nginx delivery hardening is now active and versioned in `deploy/ec2/nginx-accerio.conf`: gzip is enabled for application assets, content-hashed `chunk-*.js` files are immutable for one year, and deployment-sensitive entry files revalidate. `nginx -t` and reload passed; response headers confirm gzip and the intended cache policies.
- After delivery hardening and changing browser assertions to measure the usable report heading rather than the unrelated full-window `load` event, the full three-pass lane improved to `50/52` in 7.1 minutes. The two intermittent cold-screen misses passed an immediate targeted three-pass rerun (`7/7` including authentication). This is a substantial improvement but does not satisfy the zero-failure repeatability exit criterion.
- The non-destructive Manav-T scale audit found 16 products, 2 locations, 5 distinct named product/batch pairs, 161 inventory movements, 23 BOMs with 29 material lines, and 22 work orders with 27 material lines. This is below the agreed controlled-volume baseline; large-volume certification requires a dedicated disposable tenant or database baseline before seeding.
- A subsequent full run reached `51/52` in 6.8 minutes; the only failure was a completely blank Stock Day Book screen during the third repetition. Review identified that the workspace test bootstrap started a second navigation after waiting only for HTTP commit, allowing Angular startup and report navigation to overlap. Bootstrap now waits for `DOMContentLoaded` within the same 15-second budget before continuing.
- The post-correction full run again reached `51/52` in 7.0 minutes. The single blank-screen miss moved to the Manufacturing Summary drilldown in the first repetition, while the remaining 51 checks passed. This proves the report-specific business paths are stable but confirms a residual cold-shell startup reliability issue; Phase 8 repeatability remains open rather than being hidden with retries or a larger threshold.
...
| 9 Sep 2026 | Phase 8 | Delivery optimized; scale and clean repeat remain open | All 20 report API timings passed below 0.4 s. Nginx gzip and safe Angular caching reduced the three-pass browser lane from 10.9 to 7.1 minutes and failures from 6 to 2; both intermittent misses then passed a targeted 7/7 rerun. Manav-T contains only 16 products and 161 movements, so controlled-volume certification and one zero-failure full three-pass run remain required. | 98% |
| 9 Sep 2026 | Phase 8 | Cold-shell defect isolated | Two further complete three-pass runs each reached 51/52. Blank-screen failures moved between Stock Day Book and Manufacturing Summary, while all functional assertions passed. A bootstrap overlap was removed, but one cold-shell miss persisted under the 15-second interactive threshold. Repeatability and controlled-volume gates remain open. | 98% |
