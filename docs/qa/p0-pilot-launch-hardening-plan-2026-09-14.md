...
Status: In progress; staging identity/session and core restricted-role gates passed in all three browsers; broader role/branch isolation remains
...
| Identity, onboarding, RBAC, and isolation | In progress | Identity: Chromium 9/9 and Firefox/WebKit 12/12; restricted browser routes 27/27; live menu/API/export/cross-entity denial passed in all three browsers; branch, master export, purchase workflow-role, HRMS approval, ESS payslip isolation, financial-document attachment authorization, and payroll sensitive-object/run-action isolation passed | Failure/recovery/concurrency isolation and staging verification of the latest fixes remain | - |
