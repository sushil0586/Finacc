...
| Core accounting controls and close | Fix pending frontend deployment | The focused backend close, opening, permission, destructive-workflow, and financial-year lock gate passed 37/37. The first staging controls certificate passed 40/45: one Posting Setup row reset defect reproduced in all three browsers, while two WebKit bootstrap timeouts passed 3/3 when isolated with the corrected 30-second navigation allowance. Reset now restores the exact server-loaded mapping, enabled state, and account preference; its Angular suite passes 20/20 and the production build succeeds. Rerun the 45-test certificate after frontend deployment. |
