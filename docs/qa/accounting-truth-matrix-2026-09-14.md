...
| Dataset A staging source mutation gate | Fix ready; deployment required | The self-cleaning opening-balance flow reached account creation, then exposed a stale financial code-series cursor colliding with an existing ledger code. Allocation now skips occupied entity ledger codes atomically; `financial.tests` passes 106/106 locally. Rerun `FIN-ACCT-OPEN-001` on staging after deployment. |
