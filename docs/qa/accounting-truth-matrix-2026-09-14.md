...
| Core accounting controls and close | Complete for pilot gate | The focused backend close, opening, permission, destructive-workflow, and financial-year lock gate passed 37/37. The staging controls surface certificate passed 45/45 across Chromium, Firefox, and WebKit in 5.2 minutes. A separate isolated destructive staging certificate passed 2/2 in 59.8 seconds, proving actual close, duplicate-close rejection, closed-period posting rejection, opening carry-forward posting, duplicate-opening rejection, rollback ordering, opening-entry purge, close rollback, restored open/readiness state, and zero temporary user/entity residue. The frontend now retains the source FY after opening generation so the generated batch remains available for audit and rollback; its focused component suite passes 25/25 and awaits deployment verification. |
...
| Date | Change | Result |
| --- | --- | --- |
| 15 September 2026 | Completed the isolated destructive year-end close staging certificate and corrected the opening-generation workspace transition. | The self-cleaning certificate passed 2/2 in Chromium in 59.8 seconds and left zero temporary users or entities. Close, duplicate protection, period lock, opening generation, rollback dependency, opening purge, close rollback, and restored readiness all passed. The frontend remains on the source FY after generation; focused component tests pass 25/25 and deployment verification remains. |
