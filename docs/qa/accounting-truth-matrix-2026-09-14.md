...
Status: Baseline plus landed-cost/layered-valuation Variant 1, discount/tax/CESS Variant 2, batch/location/partial-return Variant 3, settlement/advance Variant 4, IGST/taxability Variant 5, and lifecycle/concurrency Variant 6 automated locally. Staging report and concurrency certification is in progress; one Stock Ledger deficit-ordering correction is awaiting deployment and rerun.
...
| Dataset B staging inventory reports | Correction pending deployment | The first three-browser run passed 20/24. Stock Summary reported Product X1 at 107 units while Stock Ledger reported 117 in all three engines. Root cause: Stock Ledger discarded a 10-unit outbound deficit that occurred before the first receipt and hid that zero-running row when `include_zero=false`. The ledger now preserves deficits per product/location/batch for all five valuation methods and keeps custom-period opening state immutable; local inventory reports pass 28/28 and the six Dataset B oracle tests remain green. A post-deployment 24/24 rerun is required. |
