...
| Dataset B launch cross-report gate | Complete locally | `TradingEntityAccountingTruthMatrixTests.test_dataset_b_launch_gate_inventory_tax_and_financial_statement_consistency` ties Daybook, Trial Balance, FIFO stock valuation, Trading Account, P&L, Balance Sheet, purchase register, GSTR-1, GST control ledgers, and zero party ledgers to one trading oracle. Focused gate passed locally on 16 September 2026. |
...
| Dataset C asset report exports | Complete on staging | `assets.live.spec.ts` now certifies fixed asset register, depreciation schedule, asset location/custodian, and asset events exports across Excel, CSV, PDF, and print. The staging run exposed a location/custodian Excel 500 caused by `/` in the worksheet title. The shared asset Excel writer now sanitizes invalid worksheet-title characters, local backend regression passes, and the focused staging Chromium export gate passed 1/1 on 16 September 2026 after deployment. |
...
| 16 September 2026 | Added the scenario completeness register to prevent implicit accounting-scope gaps. | Every pilot-significant accounting scenario is now classified as directly covered, covered by linked evidence, partially covered with an explicit launch gate, or intentionally post-pilot. The focused `reports.tests_accounting_truth_matrix` suite passed 14/14 after the launch-gate update. New scenarios must be added to this register before they can be treated as launch scope. |
