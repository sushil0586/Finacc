...
Status: In progress; employee/contract terminal-state integrity certified locally
...
| Guided customer setup | In progress | Deployed readiness, recovery, live scoped API, and platform-navigation checks passed in Chromium, Firefox, and WebKit | Full fresh-entity journey remains | - |
| Employee lifecycle | In progress | Modern employee and dated-contract foundation audited; terminal-state integrity passed 81/81 backend HRMS/employment tests and 29/29 contract-editor tests | Effective-dated transfer/promotion revisions, bank/emergency/document data, bulk import, maker-checker, and completeness reporting remain | - |
...
| RBAC, isolation, recovery, UX, and scale | In progress | Deployed RBAC/isolation gates passed; 108/108 local PostgreSQL payroll lifecycle/concurrency/rollback regressions passed; onboarding failure/retry passed 15/15 locally and the combined deployed staging gate passed 18/18 across Chromium, Firefox, and WebKit, with 48/48 focused Angular tests | Tax-evidence storage and three staging fixtures remain; controlled staging concurrency needs an isolated database role; session expiry and scale certification remain | - |
...
| 14 September 2026 | Cross-cutting recovery | Repeated readiness recovery and the real scoped readiness API after deploying both codebases at backend revision `7e5451e6`. | Combined staging suite passed 18/18; post-restart live readiness suite passed 6/6 across Chromium, Firefox, and WebKit; Django system check passed and public HTTPS returned 200. | Deployed frontend retry behavior and backend readiness contract agree across all supported browser engines. The staging application database role cannot create Django's isolated test database, so transaction concurrency remains certified locally rather than against customer data. | Start Phase 2 employee/employment lifecycle inventory and add a governed staging test-database path for destructive concurrency certification. |
| 14 September 2026 | Phase 2 | Audited the modern employee/contract lifecycle and added cross-record terminal-state controls. | HRMS plus legacy employment regression passed 81/81; contract editor passed 29/29; TypeScript and targeted lint passed. | Employees cannot exit or archive while an operational contract remains; terminal contracts require an end date, automatically leave payroll eligibility, and cannot be reactivated for exited/archived employees. The editor exposes the requirement before submission. | Implement an effective-dated contract revision workflow for transfer, promotion, salary-relevant assignment changes, separation, and rehire without rewriting historical records. |
