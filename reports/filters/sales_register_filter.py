...
class SalesRegisterFilter(django_filters.FilterSet):
    """
    Reusable filter base for sales-register reporting.

    Filters stay header-wise so the report remains one row per sales document.
    """
    ...
    supply_classification = django_filters.CharFilter(method="filter_supply_classification")
    ...
    def filter_invoice_type(self, queryset, name, value):
        # Current sales header does not have a separate invoice_type field.
        # Expose doc_type as the closest stable business equivalent.
        values = self._parse_enum_csv(value, enum_cls=SalesInvoiceHeader.DocType, field_name="invoice_type")
        return queryset.filter(doc_type__in=values) if values else queryset

    def filter_supply_classification(self, queryset, name, value):
        values = self._parse_enum_csv(
            value,
            enum_cls=SalesInvoiceHeader.SupplyCategory,
            field_name="supply_classification",
        )
