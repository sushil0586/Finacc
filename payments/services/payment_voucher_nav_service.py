...
class PaymentVoucherNavService:
    DEFAULT_ALLOWED_STATUSES = (
        int(PaymentVoucherHeader.Status.DRAFT),
        int(PaymentVoucherHeader.Status.CONFIRMED),
    ...
    def _scope_qs(
        *,
        ...
        return PaymentVoucherHeader.objects.filter(**filters).only(
            "id",
            "doc_no",
            "voucher_code",
            "status",
    ...
    @staticmethod
    def _to_item(obj: Optional[PaymentVoucherHeader]) -> Dict[str, Any]:
        if not obj:
            return PaymentVoucherNavService._empty_item()
        return NavItem(
            id=obj.id,
            doc_no=obj.doc_no,
    ...
    def get_prev_next_for_instance(
        instance: PaymentVoucherHeader,
        *,
        allowed_statuses: Optional[Sequence[int]] = None,
    ) -> Dict[str, Any]:
        allowed_statuses = allowed_statuses or PaymentVoucherNavService.DEFAULT_ALLOWED_STATUSES
        qs = PaymentVoucherNavService._scope_qs(
        ...
        return {
            "previous": PaymentVoucherNavService._to_item(prev_obj),
            "next": PaymentVoucherNavService._to_item(next_obj),
        }

    @staticmethod
    def get_number_navigation(instance: PaymentVoucherHeader) -> Dict[str, Any]:
        doc_type_row = (
            DocumentType.objects.filter(module="payments", default_code=instance.doc_code, is_active=True)
            .only("id")
            .first()
        )
        if not doc_type_row:
            return {
                "enabled": False,
                "reason": f"DocumentType not found for payments/{instance.doc_code}",
                "doc_type_id": None,
        ...
        nav = PaymentVoucherNavService.get_prev_next_for_instance(instance)
