...
class PaymentSettingsAPIView(ScopedEntitlementMixin, APIView):
    ...
    def _replace_lock_periods(self, rows: list[dict], *, entity_id: int, subentity_id: Optional[int]) -> None:
        ...
        for row in rows:
            ...
            PaymentLockPeriod.objects.create(entity_id=entity_id, subentity_id=subentity_id, lock_date=row["lock_date"], reason=reason)
