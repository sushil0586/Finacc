...
def _duplicate_reference_warnings(voucher: PaymentVoucherHeader) -> list[str]:
    ...
    return ["Reference already appears on another payment voucher. Please double-check before proceeding."]


def _attach_reference_feedback(response: Response, voucher: PaymentVoucherHeader) -> Response:
...
class PaymentVoucherApprovalActionSerializer(serializers.Serializer):
...
class PaymentVoucherCancelActionSerializer(serializers.Serializer):
...
def _require_payment_permission(user, *, entity_id: int, action: str, subentity_id: int | None = None):
...
class PaymentVoucherLookupAPIView(generics.GenericAPIView):
    ...
    def _base_queryset(self):
        ...
        if status_value:
            ...
            posting_statuses = {
                "DRAFT": PaymentVoucherHeader.Status.DRAFT,
                "CONFIRMED": PaymentVoucherHeader.Status.CONFIRMED,
                "POSTED": PaymentVoucherHeader.Status.POSTED,
                "CANCELLED": PaymentVoucherHeader.Status.CANCELLED,
            }
