...
ASSET_MENUS = (
    {"name": "Asset Master", "code": "asset-master", "route": "asset-master"},
    {"name": "Depreciation Run", "code": "depreciation-run", "route": "depreciation-run"},
    {"name": "Fixed Asset Register", "code": "fixed-asset-register", "route": "fixed-asset-register"},
    {"name": "Depreciation Schedule", "code": "depreciation-schedule", "route": "depreciation-schedule"},
    {"name": "Asset Events", "code": "asset-events", "route": "asset-events"},
    {"name": "Asset History", "code": "asset-history", "route": "asset-history"},
    {"name": "Asset Settings", "code": "asset-settings", "route": "asset-settings"},
)
...
def forwards(apps, schema_editor):
    ...
    existing_report_submenu_ids = list(
        Submenu.objects.filter(mainmenu_id=reports_menu.id).exclude(id__in=new_submenu_ids).values_list("id", flat=True)
    )
    ...
    existing_privileges = set(
        RolePrivilege.objects.filter(role_id__in=seeded_role_ids, submenu_id__in=new_submenu_ids).values_list("role_id", "submenu_id")
    )
    missing_privileges = []
    ...
    for legacy_role_id, rbac_role_id in legacy_rbac_roles.items():
        granted_submenu_ids = set(RolePrivilege.objects.filter(role_id=legacy_role_id, submenu_id__in=new_submenu_ids).values_list("submenu_id", flat=True))
...
def backwards(apps, schema_editor):
    ...
    submenu_ids = list(Submenu.objects.filter(submenucode__in=[spec["code"] for spec in ASSET_MENUS]).values_list("id", flat=True))
...
class Migration(migrations.Migration):
    dependencies = [
        ("rbac", "0008_add_receivables_report_menus"),
