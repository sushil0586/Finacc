...
SEED_TAG = "payables_settings_menu"
CATALOG_VERSION = "payables_settings_menu_2026_05_13"
ADMIN_ROLE_CODES = ("entity.super_admin", "admin", "entity.admin")

MENU_SPEC = {
    "code": "reports.payables.settings",
    "name": "Payables Settings",
    "route_path": "/reports/payables/settings",
    "route_name": "reports-payables-settings",
    "icon": "sliders-horizontal",
    "sort_order": 99,
    "permission_code": "reports.payables.settings.view",
}
...
def forwards(apps, schema_editor):
    ...
    group_menu = Menu.objects.filter(code="reports.reports.payables", isactive=True).first()
    ...
    existing_pairs = set(
        RolePermission.objects.filter(role_id__in=role_ids, permission_id=permission_id)
...
def backwards(apps, schema_editor):
    ...
    permission_ids = list(
        Permission.objects.filter(code=MENU_SPEC["permission_code"]).values_list("id", flat=True)
    )
    if permission_ids:
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("rbac", "0110_reconcile_financial_hub_ledger_summary_route"),
    ]
