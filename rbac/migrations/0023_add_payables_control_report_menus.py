...
def _seed_menu(apps, *, code, name, route_path, route_name, icon, sort_order):
    ...
    menu, _ = Menu.objects.update_or_create(
        ...
        defaults={
            ...
            "is_system_menu": True,
            "metadata": {"seed": "payables_control_reporting", "menu_code": code},
            "isactive": True,
        },
    )
    ...
    rows = [
        RolePermission(
            ...
            metadata={"seed": "payables_control_reporting"},
            isactive=True,
        )
...
def forwards(apps, schema_editor):
    _seed_menu(
        ...
        name="AP to GL Reconciliation",
        route_path="apglreconciliation",
    ...
    _seed_menu(
        ...
        name="Vendor Balance Exceptions",
        route_path="vendorbalanceexceptions",
...
def backwards(apps, schema_editor):
    ...
    permission_ids = list(
        Permission.objects.filter(code__in=["reports.apglreconciliation.view", "reports.vendorbalanceexceptions.view"]).values_list("id", flat=True)
    )
    menu_ids = list(
        Menu.objects.filter(code__in=["reports.apglreconciliation", "reports.vendorbalanceexceptions"]).values_list("id", flat=True)
    )
    if permission_ids:
