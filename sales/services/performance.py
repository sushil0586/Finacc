...
logger = logging.getLogger("sales.perf")


def sales_perf_enabled() -> bool:
    return bool(getattr(settings, "SALES_PERF_LOGGING", False))


def log_sales_perf(event: str, **fields) -> None:
    if not sales_perf_enabled():
        return
...
@contextmanager
def profile_sales_block(event: str, **fields):
    if not sales_perf_enabled():
        yield None
        return
